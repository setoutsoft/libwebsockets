...
static int interrupted, port = 7681, ssl_connection = LCCSCF_USE_SSL| LCCSCF_SKIP_SERVER_CERT_HOSTNAME_CHECK| LCCSCF_ALLOW_SELFSIGNED;
...
void lws_loop(lws_context* context) {
...
int main(int argc, const char **argv)
{
	...
	lws_sul_schedule(context, 0, &mcoon.sul, connect_client, 1);

	std::thread wstrd = std::thread(std::bind(lws_loop, context));
	while (!interrupted)
	{
		char message[500] = { 0 };
		...
		if (mcoon.wsi) {
			lws_write(mcoon.wsi, (unsigned char*)message, strlen(message), LWS_WRITE_TEXT);
		}
		else {
			lwsl_notice("connection not ready!");
		}
	}
